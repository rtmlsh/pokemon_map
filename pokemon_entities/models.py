...
class Pokemon(models.Model):
    title = models.CharField(max_length=200, verbose_name='Название покемона', blank=True)
    title_en = models.CharField(max_length=200, verbose_name='Название на английском', blank=True)
    title_jp = models.CharField(max_length=200, verbose_name='Название на япоском', blank=True)
    description = models.TextField(verbose_name='Описание покемона', blank=True)
    picture = models.ImageField(upload_to='pokemons_images', verbose_name='Изображение', null=True)
    previous_evolution = models.ForeignKey('self', related_name='next_evolution', on_delete=models.CASCADE, verbose_name='Предыдущая эволюция', null=True)
